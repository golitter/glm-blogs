...
import { files, parseRoute, routeHash, readLocal, writeLocal, searchFiles, suggestions, highlightedParts, type Quality, type Route } from "@/lib/blog-experience";
...
export function ImmersiveBlog() {
  ...
  function openSearch() { completeIntro(); flushSync(() => { setSimple(false); setSettings(false); setEditing(true); setSelected(-1); }); search.current?.focus(); search.current?.select(); }
  ...
  return <main className="immersive-root">
    ...
    {!ready && !simple && <div className="loading-screen" role="status"><span className="loading-bird">♧</span><p>正在打开书房…</p><button onClick={() => setSimple(true)}>先看简洁目录</button></div>}
    <nav className={`floating-nav ${awake || editing || settings || simple ? "" : "is-idle"}`} aria-label="快捷导航">
      <button aria-current={route.view === "home" ? "page" : undefined} onClick={() => navigate3D({view:"home"})}>全景</button><button onClick={() => navigate3D({view:"shelf"})}>知识目录</button><button onClick={() => navigate3D({view:"recent"})}>最近更新</button><button onClick={openSearch}>搜索 <kbd>⌘/Ctrl K</kbd></button><button onClick={() => navigate3D({...route, camera:undefined})}>重置视角</button><button aria-expanded={settings} onClick={() => {setSimple(false); setSettings(!settings); completeIntro();}}>画质</button><button onClick={explore}>随便看看</button><button onClick={() => {setSimple(!simple); completeIntro();}}>{simple ? "三维书房" : "简洁目录"}</button>
    </nav>
    ...
    {random && <aside className="random-card" role="dialog" aria-label="随机探索"><small>今天翻到这一篇</small><h2>{random.title}</h2><p>{random.path}</p><a href={random.url} target="_blank" rel="noreferrer" onClick={() => setRandom(null)}>前往 GitHub 阅读 ↗</a><button onClick={explore}>再抽一本</button><button onClick={() => setRandom(null)}>收起</button></aside>}
